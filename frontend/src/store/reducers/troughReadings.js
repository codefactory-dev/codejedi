			...
			try {
				...
				if (moduleId && lineId) {
					response = await api.get(
						`v1/troughReadings?moduleId=${moduleId}&lineId=${lineId}&date=${formatedDate}${order}`
					);
				} else if (moduleId) {
					response = await api.get(
						`v1/troughReadings?moduleId=${moduleId}&date=${formatedDate}${order}`
					);
				} else {
					response = await api.get(
						`v1/troughReadings?date=${formatedDate}${order}`
					);
				}
				const { data } = response;
				return {
...
export const loadTroughReadingAction = (payload) => {
	console.log(
		`loading troughReadingAction. this is the payload: ${JSON.stringify(
			payload
		)}`
	);
	return {
