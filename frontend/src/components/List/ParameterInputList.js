...
export default function ParameterInputList(props) {
	...
	const [inputNameHovered, setInputNameHovered] = useState({
		rowIndex: -1,
		rowState: ROW_STATES.DEFAULT,
	});
	...
	const onButtonConfirmDeleteClick = (idx) => {
		...
		if (inputNameHovered.rowState === ROW_STATES.DELETING) {
			setInputs(
				inputs.filter((input, idx) => inputNameHovered.rowIndex !== idx)
			);
	...
	const onInputNameHover = (inputIdx) => {
		if (inputNameHovered.rowIndex === inputIdx) {
			return;
		}
	...
	const onInputNameClick = (idx) => {
		setInputNameHovered({
			rowIndex: inputNameHovered.rowIndex,
			rowState: ROW_STATES.EDITING,
		});
	};
	...
	const deleteIcon = (idx) =>
		icon(idx, <CrossIcon />, theme.palette.primary.main, onButtonDeleteClick);

	const confirmDeleteIcon = (idx) =>
		icon(
			idx,
			<YesIcon />,
			theme.palette.secondary.main,
			onButtonConfirmDeleteClick
		);
		...
		<div className={classes.root}>
			...
			<div className={classes.contentContainer}>
					...
					inputs.map((input, idx) => (
						<React.Fragment key={`input-${idx}`}>
							...
							<div className={classes.contentInput}>
								<div
									className={classes.col_flex1}
									onMouseEnter={() => {
										if (!props.disabled) onInputNameHover(idx);
									}}
									onMouseLeave={() => {
										if (!props.disabled) onInputNameHover(-1);
									}}
									onClick={() => {
										if (!props.disabled) onInputNameClick(idx);
									}}
								>
									{generateInputNameComponent(input, idx)}
								</div>
								<div
									className={getInputTypeContainerClassName(input, idx)}
									onClick={() => {
										if (!props.disabled) onInputTypeClick(idx);
									}}
								>
				...
				<div
					...
					onClick={(event) => {
						if (!props.disabled) onButtonNewClick(event);
					}}
				>
