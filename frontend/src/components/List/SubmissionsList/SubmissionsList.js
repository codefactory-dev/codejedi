	...
	const getDeletionState = (input, idx) => ({
		[rowStates.DESELECTED]: (
			<div className={classes.selectedInput}>
				...
				>
					{input.casesPassed}/{input.totalCases}
				</span>
				{input && input.totalCases === 0 ? (
					''
				) : (
		...
		[rowStates.EDITING_ROW]: (
			<div className={classes.focusedInput}>
				<input id={`input-${idx}`} maxLength="20" />
			</div>
		),
					...
					>
						{inputs &&
							...
							inputs
								...
								.map((input, idx) => generateRow(input, idx))}
					</form>
				</div>
			</div>
