...
export function CodeScaffolding(entries, paramsAmount, userSolution, hiddenSolution, questionType,languageType,entryFunction="solution")
{
    try {
        let CodeScaffolding;
        ...
        switch(lowercaseLanguage){
            case languageTypes.Javascript:
                CodeScaffolding = require('../code_scaffold/javascript/index.js').CodeScaffolding;
                break;
...
export function TestScaffolding(entries, paramsAmount, testSolution, questionType,languageType,entryFunction="solution")
{
    try {
        let TestScaffolding;
        let lowercaseLanguage = languageType.toLowerCase();
        switch(lowercaseLanguage){
            case languageTypes.Javascript:
                TestScaffolding = require('../code_scaffold/javascript/index.js').TestScaffolding;
                break;
            case languageTypes.Java:
        ...
        return TestScaffolding(entries, paramsAmount, testSolution, questionType,entryFunction);
    } catch (error) {
        throw new ScaffoldError('Error creating Scaffold. '+error)
    }
}
