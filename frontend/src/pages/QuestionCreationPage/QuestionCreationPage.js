...
const QuestionCreationPage = ({
	...
	const [codeareaDisabled, setCodeareaDisabled] = useState(true);
		...
		if (currentUser) {
			async function performSave() {
				try {
					...
					const rawContext =
						typeof descriptionSubpage.editorState === 'string'
							? convertToRaw(descriptionSubpage.editorState)
							: JSON.stringify(
									convertToRaw(
										descriptionSubpage.editorState.getCurrentContent()
									)
							  );
					...
					if (error.solutionValidationError) {
						return Swal.fire(
							`Something's wrong with your solution. ${error.message}`
						);
					}
					if (error.testcasesValidationError) {
						return Swal.fire(
							`Something's wrong with your testcases. ${error.message}`
						);
					}
	...
	function testQuestion() {
		try {
			...
			const testSolution = solutionSubpage.funcSolutionCode;
			...
			const togetherText = TestScaffolding(
				structure,
					...
					<Paper square className={classes.tabs}>
						...
						>
							...
							<Tab
								disabled={
									!solutionSubpage.funcParameters ||
									!solutionSubpage.funcParameters.length > 0
								}
								disableRipple
								classes={{ root: classes.tab, selected: classes.selected }}
								label="TEST CASES"
