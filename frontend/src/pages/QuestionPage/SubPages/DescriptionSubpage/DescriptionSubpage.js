...
export default function DescriptionSubpage(props) {
	...
	useEffect(() => {
		...
		if (
			props.questionDifficulty &&
			props.questionDifficulty !== questionDifficulty
		) {
	...
	}, [questionName, questionDifficulty, questionType, editorState]);
	...
	const onQuestionTypeChange = (evt) =>
		setQuestionType(QUESTION_TYPES[evt.target.value]);
	const onQuestionDifficultyChange = (evt) =>
		setQuestionDifficulty(DIFFICULTY_TYPES[evt.target.value]);
				...
				<div className={classes.title}>
					...
					<div className={classes.colFlex1}>
						<CustomSelect
							...
							checkedOptionIndex={(() =>
								1 + getKeyIndexByValue(DIFFICULTY_TYPES, questionDifficulty))()}
					...
					<div className={classes.colFlex1}>
						<CustomSelect
							...
							checkedOptionIndex={(() =>
								1 + getKeyIndexByValue(QUESTION_TYPES, questionType))()}
